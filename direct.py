...
def run_detection(frame, model, platform):
    """Runs detection on the provided frame and returns results in that frame's coordinate system."""
    if frame is None or frame.size == 0:
        
        return [], [], []

    frame_height, frame_width = frame.shape[:2]
    inference_size_frame = cv2.resize(frame, IMG_SIZE)
    img_rgb = cv2.cvtColor(inference_size_frame, cv2.COLOR_BGR2RGB)
