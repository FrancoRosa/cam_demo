...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Tracker Age Button
        tracker_age_x = 10 + (button_width + button_gap) * 3 + button_width + button_gap + button_width + button_gap
        tracker_age_y = start_y
        if tracker_age_x < x < tracker_age_x + button_width and tracker_age_y < y < tracker_age_y + button_height:
...
def draw_buttons(frame):
    ...
    cv2.rectangle(frame, (tracker_age_x, tracker_age_y), (tracker_age_x + button_width, tracker_age_y + button_height), color_age, -1)
    cv2.rectangle(frame, (tracker_age_x, tracker_age_y), (tracker_age_x + button_width, tracker_age_y + button_height), colors.white, 2)
    ...
    cv2.putText(frame, label, (text_x_age, text_y_age), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors.white, 1, cv2.LINE_AA)
    ...
    cv2.rectangle(frame, (btn_x, btn_y), (btn_x + btn_w, btn_y + btn_h), colors.white, 2)
    label = "Mirror"
    text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
    text_x = btn_x + (btn_w - text_size[0]) // 2
    text_y = btn_y + (btn_h + text_size[1]) // 2
    cv2.putText(frame, label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors.white, 1, cv2.LINE_AA)
